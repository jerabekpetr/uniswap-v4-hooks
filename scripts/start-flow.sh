...
cleanup() {
    if [[ -n "$ANVIL_PID" ]]; then
        echo "[start-flow.sh] Zastavuju Anvil (PID $ANVIL_PID)…"
...
command -v anvil >/dev/null || { echo "[start-flow.sh] anvil nenalezen, nainstaluj Foundry." >&2; exit 1; }
command -v forge >/dev/null || { echo "[start-flow.sh] forge nenalezen, nainstaluj Foundry." >&2; exit 1; }
...
if ! cast chain-id --rpc-url "$RPC_URL" >/dev/null 2>&1; then
    echo "[start-flow.sh] Anvil neodpovídá po 15s" >&2
    exit 1
fi
echo "[start-flow.sh] Anvil připraven."

PK="0xac0974bec39a17e36ba4a6b4d238ff944bacb478cbed5efcae784d7bf4f2ff80"

echo "[start-flow.sh] Deploy V4 artefaktů…"
...
if [[ ! -f frontend/v4-addresses.json ]]; then
    echo "[start-flow.sh] frontend/v4-addresses.json nebyl vygenerován." >&2
    exit 1
fi
echo "[start-flow.sh] V4 adresy: $(cat frontend/v4-addresses.json)"

echo "[start-flow.sh] Deploy FlowScore hooku…"
...
echo "[start-flow.sh] Deploy FlowScore simulatoru…"
FLOW_HOOK_ADDRESS="$HOOK_ADDR" forge script script/flowscore/04_DeploySimulator.s.sol \
    --rpc-url "$RPC_URL" --broadcast --skip-simulation --private-key "$PK"

if [[ ! -f frontend/addresses.json ]]; then
    echo "[start-flow.sh] frontend/addresses.json nebyl vygenerován." >&2
    exit 1
fi
echo "[start-flow.sh] addresses.json:"
