...
cleanup() {
    if [[ -n "$ANVIL_PID" ]]; then
        echo "[start-jit.sh] Zastavuju Anvil (PID $ANVIL_PID)…"
...
if ! cast chain-id --rpc-url "$RPC_URL" >/dev/null 2>&1; then
    echo "[start-jit.sh] Anvil neodpovídá po 15s" >&2
    exit 1
fi
echo "[start-jit.sh] Anvil připraven."

# Použij první default anvil private key (všechny účty unlocked)
PK="0xac0974bec39a17e36ba4a6b4d238ff944bacb478cbed5efcae784d7bf4f2ff80"

echo "[start-jit.sh] Deploy V4 artefaktů…"
...
if [[ ! -f frontend/v4-addresses.json ]]; then
    echo "[start-jit.sh] frontend/v4-addresses.json nebyl vygenerován." >&2; exit 1
fi
...
echo "[start-jit.sh] Deploy Simulator…"
...
if [[ ! -f frontend/addresses.json ]]; then
    echo "[start-jit.sh] frontend/addresses.json nebyl vygenerován." >&2
    exit 1
fi
echo "[start-jit.sh] addresses.json:"
